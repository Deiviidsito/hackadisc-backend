...
try {
    ...
    if ($response1.success) {
        ...
        Write-Host "Porcentaje Completo: $($resumen.porcentaje_completo)" -ForegroundColor Green
        Write-Host "Porcentaje Simple: $($resumen.porcentaje_simple)" -ForegroundColor Yellow
...
try {
    ...
    if ($response2.success) {
        ...
        # Mostrar top 3 clientes más activos (simplificado)
        if ($response2.datos.clientes -and $response2.datos.clientes.Count -gt 0) {
            Write-Host "`nTOP 3 CLIENTES MÁS ACTIVOS:" -ForegroundColor Yellow
            for ($i = 0; $i -lt [Math]::Min(3, $response2.datos.clientes.Count); $i++) {
                ...
                Write-Host "     Flujo Completo: $($cliente.flujo_completo) casos" -ForegroundColor Green
                Write-Host "     Flujo Simple: $($cliente.flujo_simple) casos" -ForegroundColor Yellow
...
# TEST 3: Análisis completo año 2024 (sin filtro de mes)
Write-Host "`nTEST 3: Análisis completo año 2024 (todos los meses)" -ForegroundColor Yellow
...
try {
    $response3 = Invoke-RestMethod -Uri "$baseUrl/tipo-flujo/analizar" -Method POST -Body $body3 -Headers $headers
    
    if ($response3.success) {
        Write-Host "ANÁLISIS ANUAL 2024 COMPLETADO" -ForegroundColor Green
        
        $resumen = $response3.datos.resumen
        ...
        if ($totalGeneral -gt 0) {
            ...
            Write-Host "  Adopción financiamiento SENCE: $adopcionSence porciento" -ForegroundColor Green
            Write-Host "  Preferencia pago directo: $([Math]::Round(100 - $adopcionSence, 2)) porciento" -ForegroundColor Yellow
        ...
        if ($response3.datos.preferencias_clientes) {
            $pref_clientes = $response3.datos.preferencias_clientes
